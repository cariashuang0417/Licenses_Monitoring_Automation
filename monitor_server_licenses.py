...
def get_server_licenses(gis):
    """
    returns a dictionary with all licenses from hosting server
    """

    # gis.admin.servers - Server Manager object
    # gis.admin.servers.get() - Server object
    # get hosting server objects from ArcGIS Enterprise using gis.admin.servers submodule
    hosting_servers = gis.admin.servers.get(role="HOSTING_SERVER")

    # if no hosting server, return empty dictionary
    if not hosting_servers:
        return {}

    # hosting_server.system - System Manager object
    # return licenses from hosting server
    return hosting_servers[0].system.licenses


def get_license_status(gis):
    """
    Returns a dict of feature name keys with values of
    True if license is expiring/expired, or False if the license is valid through the threshold
    """

    server_license_status = {}

    # get license feature objects, which contains expiration info
    feature_licenses = get_server_licenses(gis).get('features', [])

    # loop through each license and get expiration date
    # validate if license is expiring or not
    for feature_license in feature_licenses:
...
def send_smtp(recipients, message, subject="ArcGIS Licensing Report"):
    """
